...
To begin modeling, download and run the ‘gradient_boosting_and_plots.ipnyb’ file from the SCRIPT directory. Ensure all packages are installed for this step.  After running the file, the results should mimic the ‘gradient_boost_results.png’ and ‘partial_dependence_plot.png’ in the OUTPUT folder. For the regression modeling, download and run the 'linear_regression.ipynb' file from the SCRIPT directory. Ensure all of its packages are installed as well. After running the file, the results should minic the plots in 'OUTPUT/linreg_plots/" and 'OUTPUT/linreg_results.'
